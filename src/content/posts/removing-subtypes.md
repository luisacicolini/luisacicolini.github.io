...
during the development it was easier to always carry the proofs around.
...
and with a certain *property*.

Suppose we have a definition returning a natural number `y` with a property `PROP`, we will write it as:
```lean4
...
What we will do is, first of all, write the same exact `def` without subtypes:
```lean4
def multiplyByTwo_without_subtype (x : Nat) : Nat := x * 2
```
And secondly, we will state and prove a lemma that characterizes its behaviour:
```lean4
...
```
We note two things:
1. the lemma takes the same hypotheses we required at the subtyped definition `multiplyByTwoPROP`,
  ...
  Because the hypothesis was so complex I found it very counter-intuitive to require it, and was trying to get rid of it!
...
A more complex example with recursion: let's build a definition that, given an input list of naturals
...
```
Now if we try and remove the subtype:
```lean4
...
```
We want to equip this definition with a lemma guaranteeing that the output list only contains only output numbers.
...
theorem filterList_property (lin : List Nat)
  ...
  · case cons c t tih =>
    ...
    · case _ hf =>
      apply tih
```
This proof strategy is disjoint from the one we used in the subtyped definiton.
...
a recursive call to the theorem we're writing.

With this strategy we will be sometimes asked to prove termination for the theorem - just like we do for definitions.
Recall that a proof in Lean is fundamentally a value of a certain type,
and when we prove a theorem we *construct* such value! So there really is not that much of a difference between building a proof for a lemma and building a value of a type 🤪.
...
The first difference that we need to take into account is that we need to carry around a proof that said accumulator respects the property we expect (`hacc`).
In this way, when we reach the base case of the recursion, we have both a result and a proof for that result.
Moreover, when we add a new element into the accumulator `acc' := head :: acc`, we
know that the existing `acc` already respects the property (by `hacc`),
and we simply need to show that the newly-added element `head` complies too.
...
```
This strategy is not going to work (unless `filterList_without_subtype_tailrec` is *trivial enough*™️), because we only have access to what happens when the accumulator is empty,
which is not enough!

We need to characterize the behaviour of the definition for any accumulator,
because the recursive definition actually changes it.
...
It took me quite a lot to understand this, but after thinking about it for a long time I realized that our definition is very general, and if we want to use the accumulator we have no rule pinky-promising that at the very first
...
In this last lemma the proof strategy was again the exact same strategy we used in the subtyped definition, and we recursively call the theorem (where we would otherwise call the constructor).
...
theorem filterList_without_subtype_tailrec_property_specialized
  ...
  rfl
```
One final note: when the accumulator is empty we can trivially prove `hacc` by `simp` - exactly what we did in the base case of the subtyped non-tail-recursive definition
`| [] => ⟨[], by simp⟩`.
