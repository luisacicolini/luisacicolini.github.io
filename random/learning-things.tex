...
However, none of these approaches allows to flexibly reason about and massage the semantics of dialects, to reason about the equivalence between models 
...
As a consequence, fully taking advantage of the semantics of the higher level dialects to guide the lower-level verification steps is currently complex and uniquely relies 
on the smt dialect. 


In unicornland we want : 
(1) a proper verification flow that exploits high level information to guide verification at 
lower levels and 
(2) the possibility to reason about the correctness of the lowering itsef. 
...
and the reason why this currently has not been done is because reasoning about the semantics of such a tool that encombasses many different representation working in wildly different 
...
Moreover, due to the high risks involved in making a mistake when printing a circuit, hardware verification is a fundamental step of hardware design.

For this reason, numerous solutions exist to accelerate and optimize the verification of hardware designs, as well as the verification of the overall
compilation procedure.

In particular, there are three main approaches to verification: logic (SMT and SAT solvers), automata-theoretic (Buechi Automata), and semantics-based ones (abstract interpretation). 

Currently, most hardware verification tools heavily rely on SMT and SAT solvers and focus on the verification at lower level of abstractions. 

There is always some kind of interface or abstraction that we need to use as a middle ground to encode the semantics of the problem we are trying to solve: 


we can convert the design into SAT/SMT formulae, we can extract the FSM from the SystemVerilog and then encode the FSM as a set of formulae. 

verification typically happens at the end of the design process. 

The CIRCT framework represents an innovative approach to hardware design, that applies the MLIR principles to hardware design. 
...
However, confining the verification of a design to the bottom of the design process means (1) discarding all the structural and semantics information present at higher abstraction 
...
we want to (1) let high-level information about the circuit influence and guide the verification progressive as the lowering happens 
...
it is important to define semantics carefully and to take full advantage of it. 

% why is it hard to solve

formalizign the semantics of CIRCT is generally a complex task because of how different the tasks are. 
...
the solution is that i spend the next four years of my life thinking very intensely about the semantics of circt dialects, taking extra care in formalizing their semantics such that it is correct and expressive enough 
to reason about the subtle border between different abstractions (e.g. latency insensitive vs. circuit oriented)
...
this shall open the doors to integrating further verification techniquest (e.g. automata-theoretic approaches)
