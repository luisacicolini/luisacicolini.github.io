...
% context: why is the problem important 
Hardware design is well known to be an extremely complex field. 

Lowering a design from a high-level to a low-level requires reasoning at different abstraction levels and in different ways, for example in terms of 
FSMs and dataflow circuits. 
...
Verification efforts in CIRCT currently rely on 
(1) the SMT dialect that can encode the semantics of different dialects in SMT formuale, 
in an attempt to extract useful information from higher-level abstractions and enable a progressive verification of the design
...
it is important to define semantics carefully and to take full advantage of these semantics. 

% why is it hard to solve

formalizign the semantics of CIRCT is generally a complex task 
